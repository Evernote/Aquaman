import { Dispatch, AnyAction, Store } from 'redux';
import { branchIterator } from './Branch';
export interface FlowObj {
    flowId: string;
    actionSeries: ActionSeries;
    condition?: (state: any) => boolean;
    persist: boolean;
    key?: string;
}
export interface AquamanConfig {
    onEndFlow: (flowId: string) => void;
    onStep: () => void;
    shouldPreventFlow: () => boolean | void;
    onWillChooseFlow: (flow: FlowObj) => FlowObj | false | void;
    functionMap: {
        [functionName: string]: Function;
    };
}
export declare type AquamanConfigPartial = Partial<AquamanConfig>;
export declare type ActionOrCreator = AnyAction | ((...any: any[]) => AnyAction);
export declare type MultiActionStep = (ActionOrCreator | (() => any) | AquamanStep.END_FLOW)[];
export declare type AquamanAction = ActionOrCreator | Branch | MultiActionStep | MappedFunction;
export declare type ActionSeries = AquamanAction[];
export interface Branch {
    __BRANCH__: true;
    [seriesIdx: string]: ActionSeries | boolean;
    [Symbol.iterator]?: typeof branchIterator;
}
export declare type MapReduxToConfig = (store: Store, dispatch: Dispatch<AnyAction>) => AquamanConfigPartial;
export declare enum AquamanStep {
    NEXT = "__Aquaman_NEXT__",
    PREVIOUS = "__Aquaman_PREVIOUS__",
    CLOSE = "__Aquaman_CLOSE__",
    FORCE_FLOW = "__Aquaman_FORCE_FLOW__",
    END_FLOW = "__Aquaman_END_FLOW__"
}
export interface MappedFunction {
    __Aquaman_FUNCTION__: true;
    functionName: string;
    args: any[];
}
